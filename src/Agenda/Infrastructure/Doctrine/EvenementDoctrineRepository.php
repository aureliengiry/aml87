...
namespace App\Agenda\Infrastructure\Doctrine;

use App\Agenda\Domain\Model\Evenement;
use App\Agenda\Domain\Model\Season;
use App\Agenda\Domain\Model\UrlEvenement;
 ...
 */
class EvenementDoctrineRepository extends EntityRepository
{
    /**
    ...
    private function buildRequestByFilters($query, $params = [], $filters = [])
    {
        if (isset($filters['archive'])) {
            $query->andWhere('e.archive = :archive');
            $params['archive'] = $filters['archive'];
        }

        if (isset($filters['public'])) {
            $query->andWhere('e.public = :public');
